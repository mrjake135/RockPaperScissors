...
#include "link_sig.h"
	...
	case '6': {
		if (*(argv + 6) == '1') {
			LOGIN_PRINT("sys_ctrl_shell_sw_to_block\n");
			sys_ctrl_shell_sw_to_block();
		}
		else if (*(argv + 6) == '0') {
			LOGIN_PRINT("sys_ctrl_shell_sw_to_nonblock\n");
	...
	case '7': {
		...
		ak::stack<struct test_t, 3> test_stack;

		LOGIN_PRINT("empty: %d\n", test_stack.epmty());

		test_obj.id = 0;
		test_stack.push(test_obj);
		test_obj.id = 1;
		test_stack.push(test_obj);
		test_obj.id = 2;
		test_stack.push(test_obj);
		...
		LOGIN_PRINT("top: %d %s\n", static_cast<struct test_t>(test_stack.top()).id, \
					static_cast<struct test_t>(test_stack.top()).name);
		test_stack.pop();
		LOGIN_PRINT("top: %d %s\n", static_cast<struct test_t>(test_stack.top()).id, \
					static_cast<struct test_t>(test_stack.top()).name);
		test_stack.pop();
	...
	case '9': {
		...
		for (uint8_t y = 0; y < qrcode.size; y++) {
			...
			for (uint8_t x = 0; x < qrcode.size; x++) {
				...
				LOGIN_PRINT(qrcode_getModule(&qrcode, x, y) ? "\u2588\u2588": "  ");
			}
		...
		// Show QR Code via LCD
		view_render_display_on();
		view_render.clear();

		for (uint8_t y = 0; y < qrcode.size; y++) {
			for (uint8_t x = 0; x < qrcode.size; x++) {
				view_render.drawRect(2*x, 2*y, 2, 2, qrcode_getModule(&qrcode, x, y) ? 1 : 0);
			}
		}
